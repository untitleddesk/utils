...
namespace Untitled::Math
{
  ...
  float sine(float angle) { return std::sinf(angle); }
  float cosine(float angle) { return std::cosf(angle); }
  float tangent(float angle) { return std::tanf(angle); }
  ...
  float square_root(float number)
  {
    ...
    if(number <= 0.0f)
      return 0.0f;
    __m128 value
      = _mm_set_ss(number); // Asking for CPU 'generate memory with' using SIMD
    __m128 result = _mm_sqrt_ss(value); // CPU makes the magic
    return _mm_cvtss_f32(result);       // "CPU, convert this to float"
  }
} // namespace Untitled::Math